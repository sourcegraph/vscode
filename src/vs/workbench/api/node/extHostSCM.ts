...
class ExtHostSourceControl implements vscode.SourceControl {
	...
	set revision(revision: vscode.SCMRevision) {
		...
		this._proxy.$updateSourceControl(this.handle, { revision });
	}
	...
	set setRevisionCommand(setRevisionCommand: vscode.Command | undefined) {
		...
		const internal = this._commands.converter.toInternal(setRevisionCommand);
		this._proxy.$updateSourceControl(this.handle, { setRevisionCommand: internal });
	}
	...
	set remoteResources(resources: vscode.Uri[] | undefined) {
		...
		this._proxy.$updateSourceControl(this.handle, { remoteResources: resources as URI[] });
	}
...
export class ExtHostSCM {
	 ...
	 */
	private _sourceControlRootsMap: TernarySearchTree<SourceControlHandle>;
	...
	getSourceControlForResource(resource: vscode.Uri): vscode.SourceControl | undefined {
		const handle = this._sourceControlRootsMap.findSubstr(resource.fsPath);
		if (!handle) {
			return undefined;
		}
	...
	private updateFolderSourceControlsMap(): void {
		this._sourceControlRootsMap = TernarySearchTree.forPaths<SourceControlHandle>();
		for (const { handle, rootUri } of this._sourceControlRoots) {
			this._sourceControlRootsMap.set(rootUri.fsPath, { handle });
		}
	}
}

export { ProviderHandle, GroupHandle, ResourceStateHandle, getIconPath, compareResourceThemableDecorations, compareResourceStatesDecorations, ExtHostSourceControlResourceGroup };