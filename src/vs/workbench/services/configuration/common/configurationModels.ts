...
export class Configuration extends BaseConfiguration {
	...
	compareAndUpdateOrganizationConfiguration(organization: ConfigurationModel): ConfigurationChangeEvent {
		const { added, updated, removed } = compare(this.organization, organization);
		let changedKeys = [...added, ...updated, ...removed];
		if (changedKeys.length) {
			const oldValues = changedKeys.map(key => this.getValue(key));
			super.updateOrganizationConfiguration(organization);
