...
import { GlobalActivityActionItem, GlobalActivityAction, ViewletActivityAction, ToggleViewletAction, ActivityRunActionItem } from 'vs/workbench/browser/parts/activitybar/activitybarActions';
		...
		this.globalActionBar = new ActionBar(container, {
			actionItemProvider: a => {
				if (a instanceof ViewletActivityAction) {
					return this.instantiationService.createInstance(GlobalViewletActionItem, a, ActivitybarPart.COLORS);
				}
				if (a.id === 'vs.home') {
					return this.instantiationService.createInstance(ActivityRunActionItem, a, ActivitybarPart.COLORS);
				}
				return this.instantiationService.createInstance(GlobalActivityActionItem, a, ActivitybarPart.COLORS);
			},
			orientation: ActionsOrientation.VERTICAL,
