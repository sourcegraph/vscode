...
export class ConfigureWorkspaceRecommendedExtensionsAction extends Action {
	...
	private getOrCreateExtensionsFile(): TPromise<{ created: boolean, extensionsFileResource: URI }> {
		...
		const extensionsFileResource = URI.file(paths.join(this.contextService.getWorkspace().roots[0].fsPath, '.vscode', 'extensions.json')); // TODO@Sandeep (https://github.com/Microsoft/vscode/issues/29242)
