...
export class ProcessRunnerDetector {
	...
	public detect(list: boolean = false, detectSpecific?: string): TPromise<DetectorResult> {
		if (this.taskConfiguration && this.taskConfiguration.command && ProcessRunnerDetector.supports(this.taskConfiguration.command)) {
			...
			return this.runDetection(
				new LineProcess(this.taskConfiguration.command, this.configurationResolverService.resolve(this.contextService.getWorkspace().roots[0], args), isShellCommand, options),
	...
	private resolveCommandOptions(options: CommandOptions): CommandOptions {
		...
		if (result.cwd) {
			result.cwd = this.configurationResolverService.resolve(this.contextService.getWorkspace().roots[0], result.cwd);
		}
		if (result.env) {
			result.env = this.configurationResolverService.resolve(this.contextService.getWorkspace().roots[0], result.env);
		}
		return result;
	}
