...
class TaskService extends EventEmitter implements ITaskService {
	...
	private getConfiguration(): { config: TaskConfig.ExternalTaskRunnerConfiguration; hasParseErrors: boolean } {
		let result = this.contextService.hasWorkspace() && this.contextService.getWorkspace().roots.length ? this.configurationService.getConfiguration<TaskConfig.ExternalTaskRunnerConfiguration>('tasks', { resource: this.contextService.getWorkspace().roots[0] }) : undefined;
		if (!result) {
