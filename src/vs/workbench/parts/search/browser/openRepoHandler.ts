...
export class RepoQuickOpenModel extends QuickOpenModel {
...
export class RepoEntry extends EditorQuickOpenEntry {
...
export class OpenRepoHandler extends QuickOpenHandler {
	private options: IOpenRepoOptions;
	...
	public setOptions(options: IOpenRepoOptions) {
		this.options = options;
	}

	public getResults(searchValue: string, maxSortedResults?: number): TPromise<RepoQuickOpenModel> {
		searchValue = searchValue.trim();

		// Respond directly to empty search
		if (!searchValue) {
			return TPromise.as(new RepoQuickOpenModel([]));
		}
	...
	private doFindResults(searchValue: string, cacheKey?: string, maxSortedResults?: number): TPromise<RepoQuickOpenModel> {
		...
		return this.searchService.searchRepo(this.queryBuilder.file(query)).then((complete) => {
	...
	public onOpen(): void {
		this.cacheState = new CacheState(cacheKey => this.cacheQuery(cacheKey), query => this.searchService.searchRepo(query), cacheKey => this.searchService.clearCache(cacheKey), this.cacheState);
