...
export class DynamicContentModel extends EditorModel {
...
export class DynamicContentInput extends EditorInput {

	static ID: string = 'workbench.editors.dynamicContentInput';

	private disposables: IDisposable[] = [];

	private promise: TPromise<DynamicContentModel>;
		...
		if (!this.promise) {
			...
			this.promise = this.textModelResolverService.createModelReference(this.resource)
				.then(ref => {
					...
					const markdown = this.staticContent;
					marked(markdown, { renderer });

					return TPromise.join(snippets)
						.then(refs => new DynamicContentModel(ref, refs));
				});
		}
	...
	matches(otherInput: any): boolean {
		...
		if (otherInput instanceof DynamicContentInput) {
			let otherResourceEditorInput = <DynamicContentInput>otherInput;
