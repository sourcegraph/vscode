		...
		if (this.getRecentlyOpened().workspaces.length > 0) {
			...
			jumpList.push({
				...
				items: this.getRecentlyOpened().workspaces.slice(0, 7 /* limit number of entries here */).map(workspace => {
					...
					const description = isSingleFolderWorkspaceIdentifier(workspace) ? nls.localize('folderDesc', "{0} {1}", path.basename(workspace), getPathLabel(path.dirname(workspace))) : nls.localize('srcWorkspace', "Workspace");
