...
import { Uri, commands, Disposable, window, workspace, QuickPickItem, OutputChannel, Range, WorkspaceEdit, Position, LineChange, SourceControlResourceState, TextDocumentShowOptions, ViewColumn, ProgressLocation, TextEditor, ReviewControl, CancellationTokenSource, StatusBarAlignment } from 'vscode';
...
export class CommandCenter {
	...
	private async _openResource(resource: Resource, preview?: boolean, preserveFocus?: boolean, preserveSelection?: boolean): Promise<void> {
		let left = await this.getLeftResource(resource);
		let right = await this.getRightResource(resource);
	...
	private async getLeftResource(resource: Resource): Promise<Uri | undefined> {
		switch (resource.type) {
			...
			case Status.INDEX_RENAMED:
				return this.getURI(resource.original, resource instanceof ComparisonResource ? resource.comparison.left : 'HEAD');

			case Status.COPIED:
			case Status.MODIFIED:
				return this.getURI(resource.resourceUri, resource instanceof ComparisonResource ? resource.comparison.left : '~');
